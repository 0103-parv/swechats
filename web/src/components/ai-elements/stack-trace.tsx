...
const parseStackFrame = (line: string): StackFrame => {
  ...
  if (withParensMatch) {
    ...
    if (!(functionName && filePath && lineNum && colNum)) {
      return fallbackStackFrame(trimmed)
    }
    const isInternal =
...
const fallbackStackFrame = (trimmed: string): StackFrame => ({
  columnNumber: null,
  filePath: null,
  functionName: null,
...
interface FilePathButtonProps {
  frame: StackFrame
  onFilePathClick:
    | ((filePath: string, lineNumber?: number, columnNumber?: number) => void)
    | undefined
}
